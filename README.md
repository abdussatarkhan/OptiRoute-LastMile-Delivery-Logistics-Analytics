...
# 📊 OptiRoute: Urban Last-Mile Courier Fleet & Route Density Analytics

**Urban Last-Mile Courier Fleet Data Warehouse: First-Attempt Delivery Success Rate (FADR), stops-per-hour density, driver idle time fuel penalties, and geofence SLA tracking.**

[![CI Analytics Pipeline](https://github.com/abdussatarkhan/OptiRoute-LastMile-Delivery-Logistics-Analytics/actions/workflows/ci.yml/badge.svg)](https://github.com/abdussatarkhan/OptiRoute-LastMile-Delivery-Logistics-Analytics/actions)
...
[![Maintenance](https://img.shields.io/badge/Maintained%3F-yes-brightgreen.svg?style=flat-square)](https://github.com/abdussatarkhan/OptiRoute-LastMile-Delivery-Logistics-Analytics)
...
**OptiRoute-LastMile-Delivery-Logistics-Analytics** is an enterprise-grade analytics data warehouse and business intelligence platform built for **Last-Mile Delivery & Route Telemetry**.
...
### 1️⃣ Executive KPI Command Center (Industrial High-Vis Yellow & Slate (Dark Slate))
...
```
                    ...
                    └────────────┬────────────┘
                                 ...
                                 ▼
┌──────────────────────┐   ┌─────────────────────────┐   ┌─────────────────────────┐
│     dim_drivers_courier │──►│     fact_parcel_dropoffs │◄──┤     dim_delivery_zones │
└──────────────────────┘   └─────────────┬───────────┘   └─────────────────────────┘
                                         │
       ┌─────────────────────────────────┼─────────────────────────────────┐
       ▼                                 ▼                                 ▼
┌──────────────────────┐   ┌─────────────────────────┐   ┌─────────────────────────┐
│     dim_depot_hubs   │   │     dim_vehicle_types   │   │     dim_date         │
└──────────────────────┘   └─────────────────────────┘   └─────────────────────────┘
```
...
FROM optiroute_dw.fact_parcel_dropoffs f
JOIN optiroute_dw.dim_date dt ON f.date_key = dt.date_key
...
```bash
git clone https://github.com/abdussatarkhan/OptiRoute-LastMile-Delivery-Logistics-Analytics.git
cd OptiRoute-LastMile-Delivery-Logistics-Analytics
